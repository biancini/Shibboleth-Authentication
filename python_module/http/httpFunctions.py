...
class http_methods():
    ...
    def get_url(self, url_to_read, username, password, ssl_check):
        returned_page = None
        ...
        while (return_code != 200):
            try:
                returned_page = self.get_single_url(cur_url)
                cur_url = returned_page.get_header_field("location")
                return_code = returned_page.get_return_code()
            except urllib2.HTTPError, e:
                if (e.code == 401):
                    returned_page = self.get_single_url(cur_url, username, password)
                    cur_url = returned_page.get_header_field("location")
                    return_code = returned_page.get_return_code()
                else: return_code = -1
    ...
    def get_single_url(self, url_to_read, username=None, password=None):
        cookies_string = ';'.join([("%s=%s" % (cookie.name, cookie.value)) for cookie in self.cookies])

        if (self.debug): print >> sys.stderr, 'Opening URL: ' + url_to_read
        if (self.debug): print >> sys.stderr, 'Passing the following cookies: ' + cookies_string

        if username:
            if (self.debug): print >> sys.stderr, 'Basic authentication string: ' + username + ':' + password
	...
	if (self.debug): print >> sys.stderr, 'Response code: ' + str(returned_page.get_return_code())
...
if __name__ == "__main__":
    engine = http_methods()
    engine.debug = False
    returned_page = engine.get_url("https://cloud-mi-03.mib.infn.it/secure/pam.php", "andrea", "ciaoandrea", False)
    print "Authenticated: " + str(returned_page.get_return_code() == 200)
    print "Body obtained:\n" + '\n'.join(returned_page.get_body_rows())
